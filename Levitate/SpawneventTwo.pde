class SpawneventTwo {
	...
	void spawnlevs() {
		...
		for(int i = 0; i < list.size(); i++) {
			...
			switch (r) {
				case 0: 
				levs.addSingleLev(new LevA(list.get(i), soundStringsA[int(random(soundStringsA.length-1))], main));
				break;

				case 1: 
				levs.addSingleLev(new LevB(list.get(i), soundStringsB[int(random(soundStringsB.length-1))], main));
				break;

				case 2: 
				levs.addSingleLev(new LevC(list.get(i), soundStringsA[int(random(soundStringsA.length-1))], main));
				break;
